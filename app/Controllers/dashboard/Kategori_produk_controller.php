...
class Kategori_produk_controller extends Base_controller
{
    ...
    public function index()
    {
        ...
        if ($session->get('status_login')) {
            $data = [
                ...
                'nomor_otomatis'    => $this->kategoriProdukModel->nomor_otomatis(),
    ...
    public function simpan()
    {
        ...
        $this->kategoriProdukModel->insert([
            'no_kategori_produk'   => $no_kategori_produk,
    ...
    public function ubah()
    {
        ...
        $rules = [
            ...
            'nama_kategori_produk_edit' => 'required',
        ];
    ...
    public function hapus($no_kategori_produk = null)
    {
        ...
        if ($no_kategori_produk) {
            $this->kategoriProdukModel->delete_data($no_kategori_produk);
            session()->setFlashdata('flash', [
                'type' => 'success',
    ...
    public function cetak()
    {
        ...
        $pdf->AddPage();
        $pdf->SetTitle("DAFTAR KATEGORI PRODUK");
        ...
        $pdf->Cell(80, 6, 'NO KATEGORI PRODUK', 1, 0, 'C', $fill);
        $pdf->Cell(160, 6, 'NAMA KATEGORI PRODUK', 1, 1, 'C', $fill);
        $pdf->SetFont('Arial', '', 10);
        $pdf->SetFillColor(204, 229, 250);
        $pdf->SetTextColor(0);
        $fill2 = false;

        $data = $this->kategoriProdukModel->get_kategori_produk();
        $no = 1;
        foreach ($data as $kategoriProduk) {
            $pdf->Cell(20, 6, $no, 1, 0, 'C', $fill2);
            $pdf->Cell(80, 6, $kategoriProduk['no_kategori_produk'], 1, 0, 'C', $fill2);
            $pdf->Cell(160, 6, $kategoriProduk['nama_kategori_produk'], 1, 1, 'C', $fill2);
            $fill2 = !$fill2;
            $no++;
        }
        $pdf->Output();
